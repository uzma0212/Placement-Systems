...
// mysql_select_db("placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur)
	{
		...
		{
			$sql = $connect->query("SELECT * FROM `test`.`prilogin` WHERE `Username`='$Username'");
			if($sql->num_rows== 1)
			{
				$row = $sql->fetch_assoc();
				...
				if($cur == $dbpassword)
				{
					if($query = $connect->query("UPDATE `test`.`prilogin` SET `Password` = '$Password' WHERE `prilogin`.`Username` = '$Username'"))
					{
